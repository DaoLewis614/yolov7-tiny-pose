...
class YoLov7TRT(object):
       ...
       # print(output)
        ...
        for i in range(self.batch_size):
            ...
            result_boxes, result_scores, result_classid, result_points = self.post_process(
                output[i * 57001: (i + 1) *
                       57001], batch_origin_h[i], batch_origin_w[i]
            )
            ...
            for j in range(len(result_boxes)):
               ...
               # print("the kpt corrdinate is", kpts_label)
                ...
                plot_one_box(
                    ...
                    image,
                    label="{}:{:.2f}".format(
                        categories[0], float(result_scores[j])),
      		            kpts=points,
    ...
    def non_max_suppression(self, prediction, origin_h, origin_w, conf_thres=0.75, nms_thres=0.4):
        ...
        keep_boxes = []
        while boxes.shape[0]:
            iou_output = self.bbox_iou(
                np.expand_dims(boxes[0, :4], 0), boxes[:, :4])
...
yolov7_trt = YoLov7TRT(engine_file_path)
...
try:
    ...
    #inferencing code goes here
        frame, t = yolov7_trt.infer([frame])
        frame = frame[0]
        print(frame.shape)
        print(t)
