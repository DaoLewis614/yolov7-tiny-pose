...
class IKeypoint(nn.Module):
    ...
    def __init__(self, nc=1, anchors=(), nkpt=17, ch=(), inplace=True, dw_conv_kpt=True):  # detection layer
...
class Model(nn.Module):
    def __init__(self, cfg='yolov7-tiny-pose.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect) or isinstance(m, IDetect) or isinstance(m, IKeypoint) or isinstance(m, Keypoint):
            s = 256  # 2x min stride
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        elif m in [Detect, IDetect, IKeypoint, Keypoint]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov7-tiny-pose.yaml', help='model.yaml')
