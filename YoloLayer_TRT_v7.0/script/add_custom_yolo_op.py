...
# gs load a graph
graph = gs.import_onnx(onnx.load("yolov7-tiny-pose-sim.onnx"))
...
inputs = [tensors["380"].to_variable(dtype=np.float32), 
    tensors["422"].to_variable(dtype=np.float32), 
    tensors["464"].to_variable(dtype=np.float32)]
...
# gs save a graph
onnx.save(gs.export_onnx(graph), "yolov7-tiny-pose-sim-yolo.onnx")
